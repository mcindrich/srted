...
void srt_init(srt_t *srt) { sub_list_init(&srt->subs); }
...
int srt_load_from_file(srt_t *srt, const char *fn) {
  ...
  enum PARSING_STATE { PARSING_COUNTER = 0, PARSING_TIMECODE, PARSING_TEXT };
...
void srt_free(srt_t *srt) { sub_list_free(&srt->subs); }